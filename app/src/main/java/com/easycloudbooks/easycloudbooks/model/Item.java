...
public class Item extends Application implements Constants, Parcelable {
    ...
    public Item(JSONObject jsonData) {
        ...
        try {
            ...
            if (!jsonData.getBoolean("error")) {
                ...
                if (!jsonData.isNull("rePost") && jsonData.has("rePost")) {

                    JSONObject rePostObj = jsonData.getJSONObject("rePost");
                    ...
                    if (!rePostObj.getBoolean("error")) {
                        ...
                        if (rePostObj.has("videoUrl"))
                            this.setReVideoUrl(rePostObj.getString("videoUrl"));

                        if (rePostObj.has("YouTubeVideoImg"))
                            this.setReYouTubeVideoImg(rePostObj.getString("YouTubeVideoImg"));
                        if (rePostObj.has("YouTubeVideoCode"))
                            this.setReYouTubeVideoCode(rePostObj.getString("YouTubeVideoCode"));
                        if (rePostObj.has("YouTubeVideoUrl"))
                            this.setReYouTubeVideoUrl(rePostObj.getString("YouTubeVideoUrl"));

                        if (rePostObj.has("urlPreviewTitle"))
                            this.setReUrlPreviewTitle(rePostObj.getString("urlPreviewTitle"));
                        if (rePostObj.has("urlPreviewImage"))
                            this.setReUrlPreviewImage(rePostObj.getString("urlPreviewImage"));
                        if (rePostObj.has("urlPreviewLink"))
                            this.setReUrlPreviewLink(rePostObj.getString("urlPreviewLink"));
                        if (rePostObj.has("urlPreviewDescription"))
                            this.setReUrlPreviewDescription(rePostObj.getString("urlPreviewDescription"));
                    }
                }
            }
    ...
    public void setId(long id) {
     ...
     public long getIndex() {
        return id; //ToDo: need Index Mitul-Error
    }
    ...
    public boolean isVideo() {
        return ((getAd() == 0) && (getVideoUrl() != null && getVideoUrl().length() != 0));
    }
